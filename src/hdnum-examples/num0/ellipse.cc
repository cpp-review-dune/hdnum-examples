...
public:
  ...
  void F_x(const hdnum::Vector<N> &x, hdnum::DenseMatrix<N> &result) const
  {
    ...
    result[0][1] = number_type(2.0) * x[1];
    result[1][0] = number_type(2.0) * x[0] / number_type(9.0);
    result[1][1] = number_type(2.0) * x[1];
  }
};
...
int main()
{
  ...
  u[0] = -2.0;
  u[1] = 1.0; // value for starting
  // u[0] = -1.83711730; u[1] = 0.7905694151;                    // value for
  // starting
  ...
  std::cout << "Ergebnis: " << std::setprecision(15) << u[0] << " " << u[1]
            << std::endl;

...
//! below).
template <class N> class InwardEdgeDomainFunction {
public:
  const N Phi;
...
//! Neumann values.
template <class N> class InwardEdgeBoundaryFunction {
public:
  const N Phi;
...
int main()
{
  ...
  std::cout << "Inward Edge Problem" << std::endl;
  {
    ...
    typedef LaplaceCentralDifferences<Number, Number, DomainFunction,

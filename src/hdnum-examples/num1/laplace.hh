...
private:
  typedef hdnum::SGrid<N, DF, dimension> SGrid;
  SGrid grid;
  const BoundaryFunction &bf;

public:
  // make the model
  LaplaceCentralDifferences(const hdnum::Vector<number_type> extent_,
                            const hdnum::Vector<size_type> size_,
  ...
  std::size_t size() const { return grid.getNumberOfNodes(); }

  //! set initial state including time value
  void initialize(T &, hdnum::Vector<N> &) const
  {
    // As this is a stationary model, we do not do anything
  }

  //! Returns the grid
  const SGrid &getGrid() { return grid; }
  ...
  void f(const T &t, const hdnum::Vector<N> & /* U = 0 */,
         hdnum::Vector<N> &result) const
  {
    // Reset rhs
    result = 0.;

    const hdnum::Vector<number_type> h = grid.getCellWidth();
    ...
    for (size_type n = 0; n < n_nodes; ++n) {
      ...
      if (grid.isBoundaryNode(n)) {
        ...
        const hdnum::Vector<number_type> x = grid.getCoordinates(n);
          ...
          for (int d = 0; d < dim; ++d) {
            for (int s = 0; s < 2; ++s) {
              ...
              if (size_type(bneighbor) == grid.invalid_node) {
                result[n] +=
                    number_type(side) * bf.getNeumannValue(t, x)[d] / h[d];
              }
            } // s
          }   // d
        }
  ...
  void f_x(const T &, const hdnum::Vector<N> & /* U = 0 */,
    ...
    for (size_type n = 0; n < n_nodes; ++n) {
      ...
      if (!grid.isBoundaryNode(n)) {
        for (int d = 0; d < dim; ++d) {
          ...
          for (int s = 0; s < 2; ++s)
            result[n][grid.getNeighborIndex(n, d, s * 2 - 1)] +=
                1 / h[d] / h[d];
        }
      }
      else {

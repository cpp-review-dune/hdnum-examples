...
void Poisson::assemble_system()
{
  ...
  // First assemble the matrix and right hand side by going over all elements
  // (subintervals)
  for (int j = 0; j < n_elems; j++) {
    if (p == 1) {
      ...
      // Add the local system matrix of the current (linear) element to the
      ...
      /*
       * TODO:
       * 1) The index for the grid point on the left end of each element is
       * different for p=2 2) The quadratic element matrix has to be added
       * instead of the linear one 3) Same for the right hand side 4) Remove the
       * existing two lines since everything is now implemented
       */
  ...
  if (p == 2) {
    /*
     * TODO:
     * 1) Remove further entries coupling the left and right boundary to the
     * other grid points 2) Remove the existing two lines since everything is
     * now implemented
     */
    std::cout << "Additional boundary conditions for p=2 not yet implemented"
              << std::endl;
    exit(0);
  }
...
void Poisson::output_results() const
{
  ...
  filename << "solution_h_" << h << "_alpha_" << alpha << "_a_" << a << "_p_"
           << p << ".txt";
...
int main(int argc, char **argv)
{
  if (argc < 4) {
    ...
    std::cout << "\t <h> is the grid size\n"
              ...
              << "\t <p> (optional) is the polynomial degree (by default 1)"
              << std::endl;
    return 1;
  }
  double h = 0;
  ...
  if (p > 2 || p < 0) {
    std::cout << "Polynomial degree not in the supported range, choose either "
                 "p=1 or p=2"
              << std::endl;
    return 1;
  }
